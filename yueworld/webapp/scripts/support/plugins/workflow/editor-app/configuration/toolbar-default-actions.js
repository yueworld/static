...
KISBPM.TOOLBAR = {
    ACTIONS: {
        saveModel: function (services) {
            ...
            if (parent.$app) {
                parent.$app.publish("/workflow/process/save", {
                    params: {
                        modelId: $scope.editor.getModelMetaData().modelId,
                        json: JSON.stringify($scope.editor.getJSON()),
                        svg: DataManager.serialize(svgClone)
                    }
                }, true).then(function ($resulst) {
            ...
            if (parent.$app) {
                parent.$app.publish("/workflow/set/toggleFullscreen", {}, true);
            }
