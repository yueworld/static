module.exports = function ($app) {
    ...
    $app.constant('treeConfig', {templateUrl: null})
        ...
        return {
            ...
            controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', "$timeout", function ($scope, $templateCache, $interpolate, treeConfig, $timeout) {
                ...
                $scope.headClass = function (node) {
                    ...
                    if ($scope.options.isLeaf(node, $scope) || (node.children && $scope.filterExpression && !node.children.some(function (child) {
                            if ($scope.filterExpression(child)) {
                                return true;
                            } else {
                                return false;
                            }
                ...
                $scope.dbSelectNodeLabel = function (selectedNode) {
                    if ($scope.onDbSelection && $scope.onDbSelection()) {
                        var parentNode = (this.$parent.node === this.synteticRoot) ? null : this.$parent.node;
                        var path = createPath(this)

                        $scope.onDbSelection()({
                            node: selectedNode, $path: path, $parentNode: parentNode
                        });
                    }
                ...
                $scope.selectNodeLabel = function (selectedNode) {
                    ...
                    if (!$scope.options.isLeaf(selectedNode, $scope) && (!$scope.options.dirSelectable || !$scope.options.isSelectable(selectedNode))) {
                        ...
                        if ($scope.onSelection && $scope.onSelection()) {
                            var parentNode = (transcludedScope.$parent.node === transcludedScope.synteticRoot) ? null : transcludedScope.$parent.node;
                            var path = createPath(transcludedScope)
                            $scope.onSelection()({
                                node: selectedNode,
