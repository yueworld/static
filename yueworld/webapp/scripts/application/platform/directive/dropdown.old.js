...
module.exports = function ($app) {
    // 加载样式
    require("../views/dropdown.old.css");
    /**
     * 数据字典、自动查询
     *
     ...
     *
     */
    angular.forEach(["ysFrameworkDropdown", "ysDropdown"], function (directiveName) {
            ...
            return {
                controller: ["$scope", "$q", "$compile", "$element", "$attrs", function (_$scope, $q, $compile, $element, $attrs) {
                    ...
                    if (isObjectToProperty && (!property.id || !property.text)) {
                        ...
                        $app.tip.error({message: directiveName + " Init Error: 对象型  id、text 字段不能为空！"})
                        return;
                    }
                    ...
                    if (async) {
                        ...
                        function refresh(filter) {
                            handler(angular.extend({size: option.size}, filter)).then(function ($response) {
                                ...
                                }].concat((!$response || !$response.data || !$response.data.data ? [] : $response.data.data)));
                            })
                        }

                        // 快速搜索
                        $scope.$watch("filter.term", $app.helper.watch(function (nv, ov) {
                            if (nv != ov && !filter.dq) {
                                refresh(filter);
                            } else {
                                filter.dq = false;
                        ...
                        if (!option.search) {
                            // 加载全量数据
                            refresh(filter);
                        } else if (isObjectToProperty && option.model[property.id]) {
                            // 根据条件 加载数据
                        ...
                        if (angular.isString(option.dictionary)) {
                            ...
                            $app.tip.error({message: directiveName + " Init Error: dictionary 类型错误！"})
                            return;
                        }
                    }
                    // 是否选中
